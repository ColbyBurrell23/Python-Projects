#Checking if Python has access to our Google Drive and google Sheet. It will show all the Google Sheets it has access to
# 🛠 First, we need some special tools to talk to Google Sheets.
import gspread  # This tool helps us read and write to Google Sheets using Python.
from oauth2client.service_account import ServiceAccountCredentials  # This tool helps us log in without a password.

# 🔑 Google Sheets authentication (like showing an ID card to enter)
SCOPE = [
    "https://spreadsheets.google.com/feeds",  # 📜 This allows us to read and write data in Google Sheets.
    "https://www.googleapis.com/auth/drive"   # 🚗 This allows us to open Google Drive files.
]

# 🔓 We use a secret key (stored in "keys.json") to log in as a special user (robot assistant).
CREDS = ServiceAccountCredentials.from_json_keyfile_name("keys.json", SCOPE)

# ✅ Now we are officially logged in and ready to use Google Sheets!
client = gspread.authorize(CREDS)

# 📝 Debugging step: Check what Google Sheets we can access
sheets = client.openall()  # 📄 Get a list of all Google Sheets that our robot assistant can open.

# 🔍 Print out the names of all the Google Sheets we can access
print("✅ Accessible Spreadsheets:", [sheet.title for sheet in sheets])
...
spreadsheet = client.open("Chatbot_Practice")  # Open a spreadsheet named 'chatbot'
